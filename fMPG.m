function [FC_gram, mpg] = fMPG(cycle)
    ...
    for ww = 1:length(Weng_array)
        for tt = 1:length(Teng_array)
            ...
            % Considering parallel hybrid powertrain
            Wmg1 = Weng ;
            Tmg1_comm = Tveh(tt) - Teng ;
...
%         brake_index(:,3) = round(10*brake_index(:,3))/10;
        ...
        brake_index(:,3)=cyc_mph(1:length(brake_index),2);
        ...
        while (runsim)
            ...
            while(SOCerror >=thresh) && (iter < 20) && (ISfeas)
               ...
               %[mpg, SOC_fin, FC_plot, SOC_plot, Mode_plot, t_array, ISfeas, Op_sim] = hybrid_ECMS_ATC();
                [mpg, SOC_fin,FC_plot,SOC_plot,~, ~, ISfeas] = hybrid_ECMS_ATC(Vcycle, tcycle, brake_index, mode_ind, mode_type,0, lambda, mu, mode_penal, Mveh,0,PoP);
                SOC_hist(2) = SOC_fin;
